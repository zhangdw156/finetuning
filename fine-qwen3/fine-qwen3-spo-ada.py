...
model = AutoModelForCausalLM.from_pretrained(model_path, use_cache=False, torch_dtype=torch.bfloat16)  # 改为 bfloat16
...
# 使用 add_adapter 方法
model.add_adapter(config, adapter_name="lora_1")

# 确保模型处于训练模式
model.train()

swanlab.config.update({
        "model": "Qwen3-0.6B-spo-ada",
        "prompt": PROMPT,
        "data_max_length": MAX_LENGTH,
    })
args = TrainingArguments(
    output_dir="../output/Qwen3-0.6B-spo-ada",
    ...
    report_to="swanlab",
    run_name="qwen3-0.6B-spo-ada",
    dataloader_drop_last=True,
    # 移除fp16设置，让accelerate配置来控制混合精度
