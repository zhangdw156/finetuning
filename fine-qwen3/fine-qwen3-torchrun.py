...
args = TrainingArguments(
    output_dir="../output/Qwen3-0.6B-native",
    per_device_train_batch_size=2,  # 增加批量大小以更好利用多卡
    per_device_eval_batch_size=2,
    gradient_accumulation_steps=2,  # 相应减少累积步数
    eval_strategy="steps",
    eval_steps=100,
    logging_steps=10,
    num_train_epochs=2,
    ...
    report_to="swanlab",
    run_name="qwen3-0.6B-native",
    dataloader_drop_last=True,
    fp16=True,
    remove_unused_columns=False,
    dataloader_num_workers=4,  # 增加数据加载器worker数量
    ddp_find_unused_parameters=False,  # 对LoRA训练有用
)
